 ...
 * @author asuspc
 */
public class Feed extends JFrame implements ActionListener, WindowListener{
    ...
    private RecepcaoNoticia recebeNoticia;
    private Thread tRecebeNoticia;
    
    public Feed(Usuario user){
        usuarioAtual = user;
        ...
        if (usuarioAtual != null){
            if (usuarioAtual instanceof Leitor && !usuarioAtual.login.equals("visitante")){
                try {                
                    ...
                    recebeNoticia  = new RecepcaoNoticia(server, tmNoticias, noticias);
                    tRecebeNoticia = new Thread(recebeNoticia);
                    tRecebeNoticia.start();
                } catch(Exception e) {
    ...
    public void windowClosing(WindowEvent e) {
        if (tRecebeNoticia != null)
            if (tRecebeNoticia.isAlive())
                recebeNoticia.parar();
    }
    @Override
